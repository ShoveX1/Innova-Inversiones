        ...
        <div className={`bg-white shadow-md overflow-visible flex flex-col h-min-full h-full transition-all duration-300 ${isCollapsed ? 'max-w-16' : 'max-w-sm'}`}>
            ...
            <div className="flex-1 overflow-hidden border-t border-gray-200 bg-gradient-to-r from-blue-900 to-blue-700 text-white h-min-full">
                <div className={`flex flex-col gap-2 pt-8 pb-8 transition-all duration-300 ${isCollapsed ? 'px-2' : 'p-4'}`}>
                    ...
                    {/* Dashboard */}
                    <div className={`py-4 flex items-center hover:bg-white/10 rounded-lg transition-all duration-200 cursor-pointer ${isCollapsed ? 'justify-center' : 'ps-2 flex-row gap-2'}`}>
                        <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className={`py-4 flex items-center hover:bg-white/10 rounded-lg transition-all duration-200 cursor-pointer ${isCollapsed ? 'justify-center' : 'ps-2 flex-row gap-2'}`}>
                        <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M16 4c0-1.11.89-2 2-2s2 .89 2 2-.89 2-2 2-2-.89-2-2zm4 18v-6h2.5l-2.54-7.63A1.5 1.5 0 0 0 18.54 8H17c-.8 0-1.54.37-2.01.99L14 10.5l-1.99-2.51A2.5 2.5 0 0 0 10.01 8H8.46a1.5 1.5 0 0 0-1.42 1.37L4.5 17H7v5h2v-6h2v6h2v-6h2v6h2z"/>
                        </svg>
                        {!isCollapsed && (
                            <div className="flex flex-col items-start">
